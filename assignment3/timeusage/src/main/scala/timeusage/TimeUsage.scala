...
//
//	val result = categorize(columnNames,Seq("p"->Set("t01","t03", "t11","t1801" , "t1803"),"w"->Set("t05","t1805")));
//
//	(result.get(Option("p")).toList.flatten.map { x => col(x) },result.get(Option("w")).toList.flatten.map { x => col(x) },result.filterKeys(!_.isDefined).values.toList.flatten.map ( x => col(x) )) 
//
//
//}
		def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
...
val primaryNeedsProjection: Column = primaryNeedsColumns.reduce(_+_).divide(60.00).as("primaryNeeds");
val workProjection: Column = workColumns.reduce(_+_).divide(60.00).as("work");
val otherProjection: Column = otherColumns.reduce(_+_).divide(60.00).as("other");
df
